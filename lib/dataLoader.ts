...
/** ---- モバイル専用: Filters FAB / Sheet をこのファイル内に同梱 ---- */
function MobileFilterFab({ count, onOpen }: { count: number; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
      /* 1024px 未満で常時表示（768〜1023の穴を塞ぐ） */
...
const normalize = (raw: any): Place | null => {
  ...
  const coins = Array.isArray(raw.coins) ? raw.coins : (typeof raw.coin === "string" ? [raw.coin] : undefined);
  return {
...
export default function MapShell() {
  ...
  /* 地図初期化：ズームの位置は幅と高さで決める（中央に来させない） */
  useEffect(() => {
    if (mapRef.current) return;
    const el = containerRef.current; if (!el) return;

    // StrictMode対策
    if ((el as any)._leaflet_id) { try { (el as any)._leaflet_id = null; el.innerHTML = ""; } catch {} }

    const m = L.map(el, { center: [20, 0], zoom: 2, zoomControl: true }); mapRef.current = m;

    const setZoomPos = () => {
      // 高さしきい値 < 280px なら常に bottomright
  ...
  useEffect(() => {
    ...
    const filtered = places
      .filter((p) => coin === "All" || (p.coins ?? []).includes(coin))
    ...
    filtered.forEach((p) => {
      ...
      // ====== ここだけ“恒久修正”：popupopen 毎にその popup 内でハンドラを付け直す（once: false） ======
      ...
      mk.on("popupopen", () => {
        ...
        requestAnimationFrame(() => {
          ...
          btn.addEventListener("click", handler); // once指定はしない
        });
      });
  ...
  const coinOptions = useMemo(() => ["All", ...Array.from(new Set(places.flatMap((p) => p.coins ?? []))).sort()], [places]);
  ...
  const handleApply = () => { /* 既存のURL同期関数があるならここで呼ぶ */ };
