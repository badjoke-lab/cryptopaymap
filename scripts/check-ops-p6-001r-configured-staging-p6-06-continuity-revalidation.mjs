...
const expectations = [
  ...
  [
    files.p606Contract,
    "P6-06 configured domain cutover and rollback evidence contract passed.",
  ],
];
...
if (!files.executor.includes("predecessors.every")) {
  throw new Error(
    "OPS-P6-001R must require current P6-01 through P6-05 receipts.",
  );
}
if (!files.executor.includes("expired_prior_proof")) {
  throw new Error(
    "OPS-P6-001R must support bounded renewal from expired historical proof.",
  );
}
...
if (!files.executor.includes("marker?.releaseId !== expectedReleaseId")) {
  throw new Error(
    "OPS-P6-001R must bind the public domain to the current P6-05 release.",
  );
}
