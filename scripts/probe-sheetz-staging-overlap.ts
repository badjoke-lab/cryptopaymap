...
import { entities, locations, sourceCandidates } from '../src/db/schema';
...
type OfficialStore = {
  ...
  latitude?: unknown;
  longitude?: unknown;
  features?: { cryptoFlexaPay?: unknown; cryptoCurrencyAcceptance?: unknown } | null;
};
type DirectoryPayload = { rows?: OfficialStore[] };
...
function postal(value: unknown): string {
  return text(value).split('-')[0] ?? '';
}
function addressKey(address: unknown, city: unknown, state: unknown, zip: unknown): string {
...
async function main(): Promise<void> {
  ...
  const parsed = JSON.parse(await readFile(SOURCE_FILE, 'utf8')) as DirectoryPayload;
  ...
  const addressIndex = new Map<string, typeof published>();
