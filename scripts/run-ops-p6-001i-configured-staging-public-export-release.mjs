...
function readPriorP606Topology(statusRoot, commit, now) {
  ...
  try {
    const value = JSON.parse(
      readFileSync(resolve(statusRoot, priorP606ReceiptPath), "utf8"),
    );
    receipt = isObject(value) ? value : null;
  } catch {
    receipt = null;
  ...
  const proofValid =
    ...
    generatedAt !== null &&
    expiresAt !== null &&
    receipt?.checks?.hostname?.digest ===
...
function readCurrentP606Diagnostic(statusRoot, commit, now) {
  ...
  try {
    const value = JSON.parse(
      readFileSync(resolve(statusRoot, p606DiagnosticPath), "utf8"),
    );
    receipt = isObject(value) ? value : null;
  } catch {
    receipt = null;
...
function evaluateProjectTopology(project, priorP606, diagnostic = null) {
  ...
  const customDomains = projectDomains.filter(
  ...
  return {
    ...
    approvedCustomDomainPresent: customDomains.includes(
      approvedStagingCustomDomain,
    ),
    approvedCustomDomainDigest: customDomains.includes(
      approvedStagingCustomDomain,
    )
      ? boundedHash(approvedStagingCustomDomain)
      : null,
    priorP606State: expiredPriorRevalidated
      ? "expired_prior_revalidated"
      : priorP606.state,
...
function validP6ReleaseMarker(marker) {
  ...
  return (
    marker.releaseId ===
    releaseMarker(marker.kind, marker.sourceCommit, marker.publicTreeDigest)
      .releaseId
  );
}
...
async function findReleaseDeployment(marker, attempts = 40) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const classified = await classifiedDeployments(
      marker.sourceCommit,
      marker.publicTreeDigest,
    );
    const match = classified.recognized.find(
      (item) => item.marker.releaseId === marker.releaseId,
    );
...
async function verifyExternal(baseUrl, expectedMarker, localDist) {
  ...
  for (const [path, expectedStatus, expectedType] of required) {
    const response = await fetch(
      `${baseUrl.replace(/\/$/, "")}${path}?p6_05=${Date.now()}`,
      {
        cache: "no-store",
        redirect: "manual",
      },
    ...
    if (!noPrivateLeakage(text))
      throw new Error(`external_private_leakage_${path}`);
  ...
  const localVersion = JSON.parse(
    readFileSync(join(localDist, "version.json"), "utf8"),
  );
...
async function selfTest() {
  ...
  try {
    ...
    if (publicTreeDigest(root) !== first)
      throw new Error("marker_changed_public_tree_digest");
    ...
    if (publicTreeDigest(root) === first)
      throw new Error("content_change_not_fingerprinted");
    const platformDomain = `${projectName}.pages.dev`;
    ...
    if (bootstrapTopology.status !== "passed")
      throw new Error("bootstrap_topology_failed");
    ...
    let rollbackCalls = 0;
    let waitCalls = 0;
    const alreadyVisible = await activateRelease(
      "already-visible",
      candidate.releaseId,
      {
    ...
    if (
      alreadyVisible.mode !== "already_visible" ||
      rollbackCalls !== 0 ||
      waitCalls !== 0
    ) {
    ...
    const normalRollback = await activateRelease(
      "normal-rollback",
      candidate.releaseId,
      {
    ...
    let raceChecks = 0;
    const raceConverged = await activateRelease(
      "race-converged",
      candidate.releaseId,
      {
    ...
    console.log(
      "OPS-P6-001I configured staging public export/release self-test passed.",
    );
  } finally {
...
async function execute(statusRoot, outputPath) {
  ...
  const repositoryContract =
    process.env.REPOSITORY_CONTRACT_OUTCOME === "success";
  ...
  if (preconditions) {
    ...
    try {
      ...
      if (firstDigest !== secondDigest)
        throw new Error("deterministic_generation_failed");
      ...
      const version = JSON.parse(
        readFileSync(join(candidateDist, "version.json"), "utf8"),
      );
      ...
      checks.topology = evaluateProjectTopology(
        project,
        priorP606,
        p606Diagnostic,
      );
      if (checks.topology.status !== "passed")
        throw new Error("unsafe_pages_topology");

      let classified = await classifiedDeployments(commit, firstDigest);
      if (classified.unrecognized.length > 0)
        throw new Error("unrecognized_production_deployment");
      ...
      if (
        baseline.unrecognized?.length > 0 ||
        candidate.unrecognized?.length > 0
      ) {
  ...
  for (const [name, value] of Object.entries(checks)) {
    if (isObject(value) && value.status === "failed")
      exceptions.push(`${name}:failed`);
  }
  ...
  const receipt = {
    ...
    procedure:
      "OPS-P6-001I configured staging public export and release evidence",
    checks,
    binding,
