...
async function main() {
  if (process.env.CPM_CANDIDATE_ACQUISITION_TARGET !== EXPECTED_TARGET) {
    throw new Error('Refusing reviewed Europe-west on-chain promotion outside fixed-review staging.');
  }
  ...
  if (matches.length !== 1) throw new Error('Exact reviewed on-chain Candidate/Evidence pair is missing or ambiguous.');
  ...
  if (
    ...
    throw new Error('Exact reviewed on-chain Candidate/Evidence state changed after review; refusing promotion.');
  }
  ...
  const onchainTagged = ['yes', 'only'].includes((paymentTags['payment:onchain'] ?? '').toLowerCase());
  ...
  const [[bitcoin], [bitcoinNetwork], [onchain], [assetCount], [networkCount], [methodCount]] =
    await Promise.all([
      db
        .select({ id: assets.id })
        .from(assets)
  ...
  if (!bitcoin || !bitcoinNetwork || !onchain || !sourceReady || !reviewerAuthorized) {
    throw new Error('Exact reviewed on-chain Candidate no longer satisfies promotion prerequisites.');
  }

  const requestId = await deterministicUuid(`europe-west-reviewed-onchain:request:${selected.candidateId}`);
  const entityId = await deterministicUuid(`europe-west-reviewed-onchain:entity:${selected.candidateId}`);
  const locationId = await deterministicUuid(`europe-west-reviewed-onchain:location:${selected.candidateId}`);
  const claimId = await deterministicUuid(`europe-west-reviewed-onchain:claim:${selected.candidateId}`);
  const claimAssetId = await deterministicUuid(`europe-west-reviewed-onchain:claim-asset:${selected.candidateId}`);
  ...
  const receipt = await createCandidatePromotionService(
    ...
    claim: {
      ...
      value: {
        ...
        processorId: null,
        howToPay: "Pay with Bitcoin using the merchant's on-chain Bitcoin payment option.",
