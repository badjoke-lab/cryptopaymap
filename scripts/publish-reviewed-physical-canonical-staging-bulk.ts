...
async function main() {
  ...
  const requested = Number(process.env.CPM_BULK_PUBLICATION_LIMIT ?? String(MAX_PUBLICATION_BATCH));
  if (!Number.isInteger(requested) || requested < 1 || requested > MAX_PUBLICATION_BATCH) {
    throw new Error(`CPM_BULK_PUBLICATION_LIMIT must be 1-${MAX_PUBLICATION_BATCH}.`);
  }
  ...
  const unpublished = [...unique.values()].filter(
    (row) =>
  ...
  const targets = unpublished.slice(0, requested);

  if (targets.length === 0) {
    console.log(JSON.stringify({
      target: EXPECTED_TARGET,
      eligibleReviewedClaims: unique.size,
      unpublishedReviewedClaims: 0,
  ...
  if (!publisherAuthorized) {
    console.log(JSON.stringify({
      target: EXPECTED_TARGET,
      eligibleReviewedClaims: unique.size,
      unpublishedReviewedClaims: unpublished.length,
  ...
  const requestId = await deterministicUuid(
    `reviewed-physical-canonical-publication:v2:${claimIds.slice().sort().join(',')}`,
  );
  ...
  if (publicClaimsAfter !== targets.length) {
    throw new Error(`Publication postcondition failed: expected ${targets.length}, got ${publicClaimsAfter}.`);
  }

  console.log(JSON.stringify({
    target: EXPECTED_TARGET,
    eligibleReviewedClaims: unique.size,
    unpublishedReviewedClaimsBefore: unpublished.length,
    selectedTargets: targets.length,
