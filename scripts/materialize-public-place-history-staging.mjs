...
import { publicPlaceHistoryFileSchema } from '../src/public/place-history';
...
function fallbackSummary(eventType) {
  switch (eventType) {
    case 'confirmed':
...
async function main() {
  ...
  const rawPlaces = JSON.parse(await fs.readFile(placesPath, 'utf8'));
  ...
  const placeSlugs = rawPlaces.records
    .map((record) => record?.placeSlug)
    .filter((value) => typeof value === 'string' && value.length > 0);
  if (placeSlugs.length < 1) {
    throw new Error('Expected at least one public Place before history materialization.');
  }
