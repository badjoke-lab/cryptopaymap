...
async function main() {
  ...
  if (matches.length !== 1) {
    throw new Error('Exact reviewed promoted on-chain Candidate/Evidence pair is missing or ambiguous.');
  }
  const selected = matches[0];
  if (!selected) throw new Error('Exact reviewed promoted on-chain Candidate was not selected.');
  ...
  const [promotion] = await db
    ...
    .limit(1);
  if (!promotion) throw new Error('Reviewed promoted on-chain Candidate is missing its promotion decision.');
  ...
  const [claim] = await db
    ...
    .limit(1);
  if (!claim) throw new Error('Reviewed promoted on-chain Claim is missing.');
  ...
  if (assetRows.length !== 1) {
    throw new Error('Reviewed on-chain Claim must have exactly one payment combination.');
  }
  ...
  const receipt = await createEvidenceReviewDecisionService(
    ...
    publicSummary: null,
    internalNote:
      'Exact bounded Europe-west Claim confirmed from reviewed official merchant Bitcoin on-chain payment Evidence.',
