...
async function main() {
  if (process.env.CPM_CANDIDATE_ACQUISITION_TARGET !== EXPECTED_TARGET) {
    throw new Error('Refusing reviewed Europe-west on-chain confirmation outside fixed-review staging.');
  }
  ...
  if (matches.length !== 1) {
    throw new Error('Exact reviewed on-chain promoted Candidate/Evidence pair is missing or ambiguous.');
  }
  const selected = matches[0];
  if (!selected) throw new Error('Exact reviewed on-chain promoted Candidate was not selected.');
  ...
  if (
    ...
    throw new Error('Exact reviewed on-chain Candidate/Evidence state changed before confirmation.');
  }
  ...
  const [promotion] = await db
    ...
    .limit(1);
  if (!promotion) throw new Error('Reviewed on-chain promoted Candidate is missing its promotion decision.');
  ...
  const [claim] = await db
    ...
    .limit(1);
  if (!claim) throw new Error('Reviewed on-chain promoted Claim is missing.');
  if (claim.claimStatus !== 'candidate' || claim.visibility !== 'hidden') {
    throw new Error('Reviewed on-chain Claim is not a hidden candidate Claim before confirmation.');
  }
  ...
  if (selected.evidenceClaimId === null) {
    ...
    if (!bound || bound.claimId !== claim.id) {
      throw new Error('Failed to bind reviewed on-chain Evidence to Claim.');
    }
    evidenceUpdatedAt = bound.updatedAt;
    evidenceBound = true;
  } else if (selected.evidenceClaimId !== claim.id) {
    throw new Error('Reviewed on-chain Evidence is already bound to a different Claim.');
  }
  ...
  if (assetRows.length !== 1) throw new Error('Reviewed on-chain Claim must have exactly one payment combination.');
  ...
  const requestId = await deterministicUuid(
    `europe-west-reviewed-onchain:evidence-review:${selected.candidateId}`,
  );
  ...
  const receipt = await createEvidenceReviewDecisionService(
    ...
    publicSummary: null,
    internalNote: 'Exact bounded Europe-west Claim confirmed from reviewed official merchant Bitcoin on-chain payment Evidence.',
