...
async function main() {
  ...
  const [bitcoin] = await db.select().from(assets).where(eq(assets.slug, 'bitcoin')).limit(1);
  if (!bitcoin || bitcoin.symbol !== 'BTC' || bitcoin.status !== 'active') {
    conflict('active canonical bitcoin asset is missing or inconsistent.');
  }
  ...
  const [existingNetwork] = await db
    ...
    .limit(1);
  if (existingNetwork) {
    if (existingNetwork.name !== 'Bitcoin' || existingNetwork.status !== 'active') {
