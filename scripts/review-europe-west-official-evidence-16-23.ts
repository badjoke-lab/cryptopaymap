  ...
  for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
    ...
    try {
      const response = await fetch(current, {
        ...
        headers: {
          'user-agent': 'CryptoPayMap-official-evidence-review/1.0',
...
async function main() {
  if (process.env.CPM_CANDIDATE_ACQUISITION_TARGET !== EXPECTED_TARGET) {
    throw new Error('Refusing Evidence review outside fixed-review staging.');
  }
  ...
  process.stdout.write(
    JSON.stringify({
      ...
      evidenceRows: pending.length,
      structurallyValidRows: details.filter((row) => row.structurallyValid === true).length,
      livePaymentMatches: details.filter((row) => row.liveExplicitPaymentMatch === true).length,
      liveBitcoinMentions: details.filter((row) => row.liveBitcoinMention === true).length,
      liveLightningMentions: details.filter((row) => row.liveLightningMention === true).length,
