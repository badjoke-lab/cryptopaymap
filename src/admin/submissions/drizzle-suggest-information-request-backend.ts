...
import type { SuggestInformationRequestBackend } from './information-request';

export function createDrizzleSuggestInformationRequestBackend(
  database: CryptoPayMapDatabase,
): SuggestInformationRequestBackend {
  ...
  return {
    ...
    async readRequestEvent(eventId) {
      const rows = await database
        .select({
