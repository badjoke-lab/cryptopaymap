...
import { reportSubmissionIntakeSchema } from './report-contract';

export const reportHttpMaximumBodyBytes = 128 * 1024;

const requestIdSchema = z.uuid();
const reportHttpBodySchema = z
  .object({
    challengeToken: z.string().min(1).max(2_048),
    submission: reportSubmissionIntakeSchema,
  })
  .strict();

export interface ReportHttpRuntime {
  intake: AbuseControlledSubmissionIntakeService;
  bucketDeriver: SubmissionRateLimitBucketDeriver;
}

export interface ReportHttpPagesContext<Environment = Record<string, unknown>> {
  request: Request;
  env: Environment;
...
export interface ReportHttpHandlerDependencies<Environment = Record<string, unknown>> {
  runtimeFromEnvironment(environment: Environment): ReportHttpRuntime;
  now?: () => Date;
}

class ReportHttpBodyTooLargeError extends Error {
  constructor() {
    super('Report request body is too large.');
    this.name = 'ReportHttpBodyTooLargeError';
  }
}
...
function contentLengthExceedsLimit(request: Request): boolean {
  ...
  return Number.isSafeInteger(value) && value > reportHttpMaximumBodyBytes;
}
...
async function readBoundedBody(request: Request): Promise<Uint8Array> {
  ...
  try {
    while (true) {
      ...
      if (total > reportHttpMaximumBodyBytes) {
        await reader.cancel();
        throw new ReportHttpBodyTooLargeError();
      }
      chunks.push(value);
    }
  } finally {
...
async function parseRequestBody(request: Request): Promise<z.infer<typeof reportHttpBodySchema>> {
  ...
  return reportHttpBodySchema.parse(json);
}
...
function mapSubmissionError(error: unknown): Response {
  if (error instanceof SubmissionAbuseControlError) {
    if (error.code === 'rate_limited') {
      return jsonResponse(429, { error: 'report_rate_limited' }, retryAfterSeconds(error));
    }
    if (error.code === 'abuse_request_invalid' || error.code === 'challenge_rejected') {
      return jsonResponse(400, { error: 'report_request_invalid' });
    }
  ...
  if (error instanceof SubmissionIntakeError) {
    ...
    if (error.code === 'idempotency_conflict') {
      return jsonResponse(409, { error: 'report_request_conflict' });
    }
    return jsonResponse(503, { error: 'report_unavailable' });
  }
...
export function createReportHttpHandler<Environment = Record<string, unknown>>(
  dependencies: ReportHttpHandlerDependencies<Environment>,
) {
  const now = dependencies.now ?? (() => new Date());

  return async (context: ReportHttpPagesContext<Environment>): Promise<Response> => {
    if (mediaType(context.request) !== 'application/json') {
      return jsonResponse(415, { error: 'report_media_type_unsupported' });
    }
    if (contentLengthExceedsLimit(context.request)) {
      return jsonResponse(413, { error: 'report_request_too_large' });
    }
    ...
    let parsedBody: z.infer<typeof reportHttpBodySchema>;
    try {
      parsedBody = await parseRequestBody(context.request);
    } catch (error) {
      if (error instanceof ReportHttpBodyTooLargeError) {
        return jsonResponse(413, { error: 'report_request_too_large' });
      }
      return jsonResponse(400, { error: 'report_request_invalid' });
    }
