...
import type { SuggestHttpRuntime } from './suggest-http';
import { createSuggestSubmissionPrivateIntakeService } from './suggest-intake-service';
...
export type SuggestHttpEnvironment = Readonly<
...
export class SuggestHttpEnvironmentConfigurationError extends Error {
  constructor() {
    super('Suggest HTTP environment configuration is unavailable.');
    this.name = 'SuggestHttpEnvironmentConfigurationError';
  }
}
...
export function createSuggestHttpRuntimeFromEnvironment(
  environment: SuggestHttpEnvironment,
): SuggestHttpRuntime {
  try {
    ...
    if (!isDurableObjectNamespace(environment.SUBMISSION_RATE_LIMIT_BUCKETS)) {
      throw new SuggestHttpEnvironmentConfigurationError();
    }

    const database = createDatabase(databaseEnvironment.DATABASE_URL);
    const privateIntake = createSuggestSubmissionPrivateIntakeService({
    ...
    if (error instanceof SuggestHttpEnvironmentConfigurationError) throw error;
    throw new SuggestHttpEnvironmentConfigurationError();
  }
}
