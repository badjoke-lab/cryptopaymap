...
import type {
  ReportCanonicalTargetContextBackend,
  ReportCanonicalTargetMaterial,
} from './report-target-context';

function iso(value: Date | null): string | null {
  return value?.toISOString() ?? null;
}

export function createDrizzleReportTargetContextBackend(
  database: CryptoPayMapDatabase,
): ReportCanonicalTargetContextBackend {
  return {
    async loadTarget(targetType, targetId): Promise<ReportCanonicalTargetMaterial | null> {
      ...
      let location: ReportCanonicalTargetMaterial['location'] = null;
      if (locationId !== null) {
