...
function readBrowserTarget(fallback: ResolvedTarget): ResolvedTarget {
  ...
  const targetType =
    requestedType === 'entity' || requestedType === 'location'
      ? requestedType
      : fallback.targetType;
...
export function ConfiguredPhotoForm({
  ...
  useEffect(() => {
    ...
    setTarget(readBrowserTarget({ targetType: initialTargetType, targetId: initialTargetId }));
  ...
  if (state === 'loading') {
    return (
      <StatePanel
        tone="loading"
        title="Preparing the secure photo form"
        description="Loading the private upload and verification configuration for this environment."
      />
    );
  }
  ...
  if (state === 'error' || configuration === null) {
    return (
      <StatePanel
        tone="error"
        title="Photo form unavailable"
        description="The private upload environment is not ready. No photo or public data was changed. Please try again later."
      />
    );
  }
