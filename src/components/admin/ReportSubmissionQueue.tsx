...
import {
  reportSubmissionQueueResponseSchema,
  type ReportSubmissionQueueItem,
} from '../../admin/submissions/report-queue';
import { Button } from '../ui/Button';

type QueueState =
  | { status: 'loading' }
  | { status: 'ready'; items: ReportSubmissionQueueItem[]; nextCursor: string | null }
  | { status: 'loading_more'; items: ReportSubmissionQueueItem[]; nextCursor: string | null }
  | { status: 'denied' }
  | { status: 'unavailable' }
  | { status: 'error' };
...
function reportLabel(item: ReportSubmissionQueueItem): string {
  if (item.reportKind === 'payment_report') {
    return `Payment ${item.paymentResult?.replaceAll('_', ' ') ?? 'report'}`;
  }
  return item.problemType?.replaceAll('_', ' ') ?? 'Problem report';
}

function QueueCard({ item }: { item: ReportSubmissionQueueItem }) {
  return (
    <article className="rounded-card border border-border bg-surface p-5 shadow-sm">
      ...
      <div className="mt-5">
        <a
          ...
          href={`/admin/submissions/report-detail?id=${encodeURIComponent(item.id)}`}
        >
          Review report
        </a>
      </div>
...
export function ReportSubmissionQueue() {
  ...
  const loadQueue = useCallback(async (cursor?: string, append = false) => {
    ...
    const url = new URL('/admin/api/reports', window.location.origin);
    ...
    try {
      ...
      const result = reportSubmissionQueueResponseSchema.safeParse(await response.json());
      if (!result.success) {
        setState({ status: 'error' });
        return;
      }
  ...
  return (
    <div aria-live="polite">
      {state.status === 'loading' ? (
        <StatusPanel
          title="Loading payment and problem reports"
          description="The protected queue is loading bounded normalized summaries only. Restricted evidence and private operational data are not requested."
      ...
      {state.status === 'error' ? (
        <StatusPanel
          title="Report queue response could not be verified"
          description="The queue response was incomplete or invalid. No unverified report values are displayed."
          icon={<AlertTriangle className="size-5" />}
          action={
            <Button variant="secondary" onClick={() => void loadQueue()}>
              Retry queue
      ...
      {pageState && items.length === 0 ? (
        <StatusPanel
          title="No actionable payment or problem reports"
          description="The bounded report queue returned a valid empty page."
          icon={<CheckCircle2 className="size-5" />}
        />
      ) : null}
      {items.length > 0 ? (
        <section aria-labelledby="report-submission-results-title">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="m-0 text-sm font-semibold text-brand-700">Protected queue</p>
              <h3
                id="report-submission-results-title"
