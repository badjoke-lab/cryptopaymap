...
export function ReconfirmationQueue() {
  ...
  return (
    <section aria-labelledby="rechecks-results-title">
      <h2 id="rechecks-results-title" className="text-2xl font-semibold text-ink">
        Reconfirmation Claims
      </h2>
      <div className="mt-5 grid gap-4">
        {items.map((item) => (
          ...
          >
            ...
            <a
              className="mt-4 inline-flex min-h-11 items-center rounded-control border border-border bg-white px-4 text-sm font-semibold text-brand-700"
              href={`/admin/rechecks/detail/?id=${encodeURIComponent(item.id)}`}
            >
              Review Claim
            </a>
          </article>
