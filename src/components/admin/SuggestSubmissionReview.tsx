    ...
    <div className="grid gap-6">
      ...
      <Section title="Normalized proposal">
        <dl className="grid gap-4 sm:grid-cols-2">
          ...
          <Value
            ...
            value={projection.entity.websiteUrl ? <a href={projection.entity.websiteUrl}>{projection.entity.websiteUrl}</a> : 'Not supplied'}
          />
          <Value label="Country" value={projection.entity.countryCode ?? projection.place?.countryCode ?? 'Not supplied'} />
          {projection.place ? (
            <>
          ...
          {projection.categories.length === 0 ? (
            ...
            <ul className="mt-2 grid gap-2 text-sm text-ink">
              {projection.categories.map((category) => (
                <li key={category.slug}>{category.slug}{category.isPrimary ? ' · primary' : ''}</li>
              ))}
            </ul>
          )}
        </div>
      ...
      <Section title="Payment proposals">
        <div className="grid gap-4">
          {projection.paymentProposals.map((payment, index) => (
            <article key={`${payment.assetSlug ?? 'unknown'}-${payment.networkSlug ?? 'unknown'}-${index}`} className="rounded-control border border-border bg-canvas p-4">
        ...
        {projection.evidenceLinks.length === 0 ? (
          ...
          <ul className="grid gap-3">
            {projection.evidenceLinks.map((evidence) => (
              <li key={evidence.url} className="rounded-control border border-border bg-canvas p-4 text-sm">
                <a className="break-all font-medium" href={evidence.url} rel="noreferrer" target="_blank">{evidence.url}</a>
      ...
      <Section title="Review signals">
        <p className="m-0 text-sm leading-6 text-muted">
          Signals are bounded review hints only. Zero results are explicitly non-conclusive and no automatic duplicate or target decision is made.
        </p>
            ...
            {signals.candidateSignals.length === 0 ? (
              ...
              <ul className="mt-3 grid gap-3">
                {signals.candidateSignals.map((signal) => (
                  <li key={signal.candidateId} className="rounded-control border border-border bg-canvas p-4 text-sm">
                    <a href={`/admin/candidates/detail?id=${encodeURIComponent(signal.candidateId)}`} className="font-semibold">Candidate {signal.candidateId.slice(0, 8)}…</a>
                    <p className="mt-2 text-muted">{signal.reasons.map((reason) => `${reason.reason} (${reason.strength})`).join(', ')}</p>
                  </li>
                ))}
              </ul>
            )}
            ...
            {signals.canonicalTargetSignals.length === 0 ? (
              ...
              <ul className="mt-3 grid gap-3">
                {signals.canonicalTargetSignals.map((signal) => (
                  <li key={signal.target.canonicalPath} className="rounded-control border border-border bg-canvas p-4 text-sm">
                    <a href={signal.target.canonicalPath} className="font-semibold">{signal.target.entity.name}</a>
                    <p className="mt-2 text-muted">{signal.reasons.join(', ')} · {signal.strength}</p>
                  </li>
                ))}
              </ul>
            )}
      ...
      <Section title="Workflow history">
        <ol className="grid gap-3">
          {events.map((event, index) => (
            <li key={`${event.createdAt}-${index}`} className="rounded-control border border-border bg-canvas p-4 text-sm">
              <p className="font-semibold text-ink">{event.action.replaceAll('_', ' ')}</p>
              <p className="mt-1 text-muted">
                {event.fromStatus ? `${event.fromStatus} → ` : ''}{event.toStatus} · {event.actorType} · {new Date(event.createdAt).toLocaleString()}
              </p>
              {event.reasonCode ? <p className="mt-1 text-muted">Reason: {event.reasonCode}</p> : null}
            </li>
          ))}
        </ol>
        {eventsTruncated ? <p className="mt-3 text-sm text-muted">History is truncated to the first 100 events.</p> : null}
      </Section>
    </div>
  );
}
...
export function SuggestSubmissionReview() {
  ...
  const submissionId = typeof window === 'undefined' ? null : new URL(window.location.href).searchParams.get('id');
  ...
  if (state.status === 'loading') {
    return <StatusPanel title="Loading Suggest review" description="The protected reviewer workspace is loading the normalized proposal and bounded read-only signals." icon={<RefreshCw className="size-5 animate-spin motion-reduce:animate-none" />} />;
  }
  if (state.status === 'missing_id') {
    return <StatusPanel title="Submission ID required" description="Open a Suggest submission from the protected queue." icon={<AlertTriangle className="size-5" />} action={<a href="/admin/submissions">Return to queue</a>} />;
  }
  if (state.status === 'denied') {
    return <StatusPanel title="Submission review access denied" description="Your verified administration identity does not have the Submission read capability." icon={<ShieldAlert className="size-5" />} />;
  }
  if (state.status === 'not_found') {
    return <StatusPanel title="Suggest submission not found" description="No reviewable Suggest submission matched this protected identifier." icon={<AlertTriangle className="size-5" />} action={<a href="/admin/submissions">Return to queue</a>} />;
  }
  if (state.status === 'unavailable') {
    return <StatusPanel title="Suggest review unavailable" description="The protected detail or signal backends could not complete safely. No partial review workspace is displayed." icon={<AlertTriangle className="size-5" />} action={<Button variant="secondary" onClick={() => void loadDetail()}>Retry review</Button>} />;
  }
  if (state.status === 'error') {
    return <StatusPanel title="Suggest review response could not be verified" description="The response was incomplete or invalid. No unverified reviewer values are displayed." icon={<AlertTriangle className="size-5" />} action={<Button variant="secondary" onClick={() => void loadDetail()}>Retry review</Button>} />;
  }
