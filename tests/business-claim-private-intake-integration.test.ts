...
function validBusinessClaim(): any {
  return {
    ...
    originalPayload: {
      ...
      verification: {
        ...
        officialContactEmail: 'representative@merchant.example',
...
function contactProtector(protectEmail = vi.fn()): SubmissionContactProtector {
  return {
    async protectEmail(email, received) {
      ...
      return {
        encryptedEmail: 'encrypted-submitter-contact',
...
describe('P5-04B business claim private intake integration', () => {
  it('stores private original Claim material and a review-safe normalized projection', async () => {
    const { persistence, intake } = claimFoundation();

    const receipt = await intake.submit(requestId, validBusinessClaim(), receivedAt);
    ...
    expect(stored).toMatchObject({
      ...
      relationship: 'owner_or_authorized_representative',
      originalPayload: {
        originalPayload: validBusinessClaim().originalPayload,
        evidenceLinks: validBusinessClaim().evidenceLinks,
        acknowledgements: validBusinessClaim().acknowledgements,
      },
      normalizedPayload: {
        targetType: 'entity',
        targetId,
    ...
    expect(normalized).not.toContain('encrypted-submitter-contact');
  });

  it('protects optional submitter contact outside original and normalized payloads', async () => {
    ...
    expect(protectEmail).toHaveBeenCalledWith('submitter@example.test', receivedAt);
    expect(stored?.contact).toMatchObject({
      encryptedEmail: 'encrypted-submitter-contact',
      emailHash: 'e'.repeat(64),
      contactAllowed: true,
    });
    expect(JSON.stringify(stored?.originalPayload)).not.toContain('submitter@example.test');
    expect(JSON.stringify(stored?.normalizedPayload)).not.toContain('submitter@example.test');
  });

  it('rejects invalid Claim input before contact protection or persistence', async () => {
    const protectEmail = vi.fn();
    const { persistence, intake } = claimFoundation(contactProtector(protectEmail));
    const invalid = validBusinessClaim();
    invalid.originalPayload.verification.officialContactEmail = 'person@unrelated.example';
