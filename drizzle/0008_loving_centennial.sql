...
CREATE TABLE "verification_events" (
	...
	CONSTRAINT "verification_events_operator_actor" CHECK ("verification_events"."actor_type" <> 'operator' or "verification_events"."actor_id" is not null),
	...
	CONSTRAINT "verification_events_status_event_shape" CHECK ("verification_events"."event_type" not in ('confirmed', 'reconfirmed', 'marked_stale', 'ended', 'restored') or ("verification_events"."from_visibility" is null and "verification_events"."to_visibility" is null)),
	CONSTRAINT "verification_events_visibility_event_shape" CHECK ("verification_events"."event_type" not in ('hidden', 'unhidden') or ("verification_events"."from_status" is null and "verification_events"."to_status" is null)),
	CONSTRAINT "verification_events_corrected_shape" CHECK ("verification_events"."event_type" <> 'corrected' or ("verification_events"."from_status" is null and "verification_events"."to_status" is null and "verification_events"."from_visibility" is null and "verification_events"."to_visibility" is null and ("verification_events"."public_summary" is not null or "verification_events"."internal_note" is not null))),
